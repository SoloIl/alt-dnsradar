...
Alt DNSRadar — это лёгкая кросс-платформенная CLI-утилита на Go, которая находит альтернативные IP-адреса для домена с помощью EDNS Client Subnet (ECS) probing и ранжирует доступные результаты по реальной TCP-задержке соединения. Спасибо Ori from ntc.party за идею с ECS-сканированием.
...
- Сохранение связи ECS-префикса с endpoint для лучших результатов; `-v`
  показывает все префиксы без дополнительных geo lookup
...
2. Затем она выполняет ECS-сканирование по 540 публичным подсетям, сохраняя информацию о том, какие ECS-префиксы вернули каждый IP.
...
5. На выходе программа показывает самые быстрые доступные endpoint и соответствующие им ECS-префиксы.
...
- Вывод ECS-префиксов использует уже собранные DNS-данные и не обращается к `ipinfo.io`.
- Утилита должна оставаться лёгкой и переносимой.

## Готовые пакеты

Готовые пакеты для основных платформ будут доступны на странице GitHub Releases.

Ссылки на release-артефакты:
...
- Поэтому обычный запуск делает не более 5 запросов к `ipinfo.io`. Флаг `-v`
  меняет только подробность ECS-вывода и не увеличивает число запросов.
...
- ECS-префикс, один раз вернувший endpoint, является кандидатом, а не гарантией:
  CDN-маппинг может меняться, поэтому перед внедрением результат нужно
